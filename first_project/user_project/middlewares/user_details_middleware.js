...
const delete_image = require("./delete_image_file.js");
...
module.exports.update_request_validation = (req, res, next) => {
  ...
  if (error.length) {
    display.end_result(res, 400, {
      ...
      message: `the request contains ${error}`,
    });
    return;
  }
  next();
  ...
  if (req_data.role == 0) {
    ...
    if (req_data.user_id != id) {
      display.end_result(res, 403, {
        message: "sorry you don't have the access to update other's details",
      });
      return;
    }
  } else {
    display.end_result(res, 403, {
      message: "sorry you don't have the access to update user's details",
    });
    return;
  }
  next();
