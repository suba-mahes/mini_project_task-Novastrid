    ...
    if (data) {
      ...
      if (data.is_active) {
        ...
        display.end_result(res, 401, {
          message: "Can not login into a in-active user account",
        });
      }
    } else {
...
module.exports.forget_password = async (req, res) => {
  try {
    ...
    if (data) {
      const token = jwt.sign({ email_id: email_id }, config.secret_key, {
        expiresIn: "1h",
      });
      ...
      await transporter.sendMail(mailOptions, (error, info) => {
        if (error) {
          ...
          display.end_result(res, 200, {
            message: `Email sent is ${result[2]}`,
            token_to_reset_password: token,
          });
        }
      });
      //   display.end_result(res, 200, {
      //     message: `Email sent is OK`,
      //     token_to_reset_password: token,
      //   });
    } else {
      ...
      display.end_result(res, 404, {
        ...
        registeration_url: register_url,
      });
      return;
    }
  } catch (error) {
...
module.exports.reset_password = async (req, res) => {
  try {
    ...
    if (data) {
      ...
      display.end_result(res, 200, {
        message: "password is reseted in successfully",
        login_url: login_url,
      });
    } else {
      const register_url = `${req.protocol}://${req.get("host")}/register`;
